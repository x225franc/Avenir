...
export class CreditRepository implements ICreditRepository {
	...
	async findByUserId(userId: UserId): Promise<Credit[]> {
		const query = `SELECT * FROM credits WHERE user_id = $1 ORDER BY created_at DESC`;
		const result = await pool.query(query, [parseInt(userId.value)]);

		return result.rows.map((row: CreditRow) => this.mapRowToCredit(row));
	}

	async findByAdvisorId(advisorId: UserId): Promise<Credit[]> {
		const query = `SELECT * FROM credits WHERE advisor_id = $1 ORDER BY created_at DESC`;
		const result = await pool.query(query, [parseInt(advisorId.value)]);
	...
	async delete(id: CreditId): Promise<void> {
		const query = `DELETE FROM credits WHERE id = $1`;
